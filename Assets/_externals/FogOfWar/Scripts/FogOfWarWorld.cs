...
namespace FOW
{
    ...
    public class FogOfWarWorld : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (UseMiniMap)
                Graphics.Blit(null, MiniMapTexture, MiniMapMat);
        }
        #endregion
        ...
        public void Initialize()
        {
            ...
            if (UseMiniMap)
            {
                MiniMapMat = new Material(Shader.Find("Hidden/FullScreen/FOW/MiniMap"));
                InitMiniMapRT();

                UpdateMaterialProperties(MiniMapMat);
                MiniMapMat.SetBuffer(Shader.PropertyToID("_ActiveCircleIndices"), indicesBuffer);
                MiniMapMat.SetBuffer(Shader.PropertyToID("_CircleBuffer"), circleBuffer);
                MiniMapMat.SetBuffer(Shader.PropertyToID("_ConeBuffer"), anglesBuffer);
                MiniMapMat.EnableKeyword("IGNORE_HEIGHT");
            }
            SetMaterialBounds();
        ...
        public void InitMiniMapRT()
        {
            MiniMapTexture = new RenderTexture(FowResX, FowResY, 0);
            MiniMapTexture.Create();
            UIImage.texture = MiniMapTexture;
        }
        ...
        public void UpdateMaterialProperties(Material material)
        {
            ...
            material.SetColor(materialColorID, material == MiniMapMat ? MiniMapColor : UnknownColor);
            ...
            material.DisableKeyword("MIN_DIST_ON");
            if (AllowMinimumDistance)
                material.EnableKeyword("MIN_DIST_ON");
        ...
        void SetMaterialBounds()
        {
            ...
            if (UseMiniMap && MiniMapMat != null)
                MiniMapMat.SetVector("_worldBounds", new Vector4(WorldBounds.size.x, WorldBounds.center.x, WorldBounds.size.z, WorldBounds.center.z));
        }
        ...
        void SetNumCircles()
        {
            ...
            if (UseMiniMap)
                MiniMapMat.SetInt(numCirclesID, numRevealers);
            foreach (PartialHider hider in PartialHiders)
    ...
    public class FogOfWarWorldEditor : Editor
    {
        ...
        void OnSceneGUI()
        {
            ...
            if (fow.UseWorldBounds || fow.UseMiniMap)
            {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (newUseMiniMap)
            {
                ...
                int newMiniMapResX = EditorGUILayout.IntSlider("Mini Map Res X: ", MiniMapResX, 128, 2048);
                ...
                int newMiniMapResY = EditorGUILayout.IntSlider("Mini Map Res Y: ", MiniMapResY, 128, 2048);
                ...
                RawImage newReference = (RawImage)EditorGUILayout.ObjectField("Raw Image: ", oldReference, typeof(RawImage), true);
