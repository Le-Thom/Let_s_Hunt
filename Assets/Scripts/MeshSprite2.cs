...
namespace Valax321.MeshSprites
{
    ...
    public class MeshSprite2 : MonoBehaviour
    {
        ...
        private void GenerateMeshForSprite(MeshFilter filter, Sprite newSprite)
        {
            ...
            var mesh = new Mesh
            {
                ...
                vertices = newSprite.vertices.Select(x => new Vector3(x.x, x.y, 0)).ToArray(), //Convert(x => new Vector3(x.x, x.y, 0)),
                triangles = newSprite.triangles.Select(x => (int)x).ToArray(), //Convert(x => (int)x),
                uv = newSprite.uv
            };
